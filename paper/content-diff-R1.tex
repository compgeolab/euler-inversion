...
\item \textbf{Correlation of the estimated depth and the structural index:}
    ...
    \DIFaddbegin \DIFadd{Another approach is that of \mbox{%DIFAUXCMD
\citet{Mushayandebvu2004}}\hskip0pt%DIFAUXCMD
, who exploit the
...
\DIFdelbegin \DIFdel{We aim to tackle the issues of
sensitivity to noise and structural indexestimation }\DIFdelend \DIFaddbegin \DIFadd{Euler deconvolution and its variants are also know to struggle with models that
...
\mbox{%DIFAUXCMD
\citet{Gerovska2010} }\hskip0pt%DIFAUXCMD
were developed based on the similarity transform.
...
\DIFadd{We aim to tackle some of these issues }\DIFaddend by reformulating the inverse problem of
...
$\partial_\alpha$ is the derivative operator in the $\alpha$ \DIFdelbegin \DIFdel{dimension}\DIFdelend \DIFaddbegin \DIFadd{direction}\DIFaddend ,
...
\begin{equation}
  \DIFdelbegin %DIFDELCMD < \hat{\mathbf{p}} %%%
\DIFdelend \DIFaddbegin \DIFadd{\mathbf{p} }\DIFaddend = \left(\mathbf{A}^T\mathbf{A}\right)^{-1}
  \mathbf{A}^T\mathbf{c}
  \ .
  \label{eq:deconv-p}
\end{equation}
...
\noindent
The covariance matrix of the \DIFdelbegin \DIFdel{solution }\DIFdelend \DIFaddbegin \DIFadd{parameters }\DIFaddend $\mathbf{C}_p$ is obtained through
...
\DIFaddbegin \DIFadd{In practice, the weight matrix is usually diagonal because covariances of
...
Applying the identity \DIFaddbegin \DIFadd{above }\DIFaddend to Equation~\ref{eq:normal} with
$\mathbf{g} = \mathbf{\Delta d}_k$ and
\DIFdelbegin \DIFdel{$\mathbf{h} = \left[\boldsymbol{\Delta\lambda}_k \quad \mathbf{\Delta p}_k\right]^T$
}\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{h} = \left[\boldsymbol{\Delta\lambda}_k^T \quad \mathbf{\Delta p}_k^T\right]^T$
}\DIFaddend leads to
...
vector.
Applying the identity \DIFaddbegin \DIFadd{in Equation~\ref{eq:identity} }\DIFaddend once more to the equation
system above leads to a solution for the parameter correction vector

\begin{equation}
  \DIFdelbegin %DIFDELCMD < \hat{\mathbf{\Delta p}}%%%
\DIFdelend \DIFaddbegin \DIFadd{\mathbf{\Delta p}}\DIFaddend _k =
...
\DIFdelbegin \DIFdel{$\hat{\mathbf{\Delta p}}_k$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{\Delta p}_k$ }\DIFaddend from Equation~\ref{eq:normal2}, which results in

\begin{equation}
  \DIFdelbegin %DIFDELCMD < \hat{\boldsymbol{\Delta\lambda}}%%%
\DIFdelend \DIFaddbegin \boldsymbol{\Delta\lambda}\DIFaddend _k =
  \mathbf{Q}_k^{-1}
...
correction as a function of \DIFdelbegin \DIFdel{$\hat{\mathbf{\Delta p}}_k$
}\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{\Delta p}_k$
}\DIFaddend 

\begin{equation}
  \DIFdelbegin %DIFDELCMD < \hat{\mathbf{\Delta d}}%%%
\DIFdelend \DIFaddbegin \DIFadd{\mathbf{\Delta d}}\DIFaddend _k =
  \mathbf{r}_k -
...
\DIFaddbegin \noindent
\DIFadd{It is worth mentioning that the Lagrange multipliers
...
\DIFaddend The covariance matrix of $\mathbf{p}$ is used to rank and filter solutions
...
The \DIFdelbegin \DIFdel{covariance matrix of }\DIFdelend \DIFaddbegin \DIFadd{propagation requires }\DIFaddend the observed data \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{covariance matrix $\mathbf{C}_d$,
which can be }\DIFaddend approximated by
...
\DIFaddbegin \DIFadd{In all of our synthetic data tests, we used the range $\eta_{min}
...
separately by the variance of the $z_o$ estimate\DIFdelbegin \DIFdel{(from }\DIFdelend \DIFaddbegin \DIFadd{, which can obtained by
selecting the corresponding element from the covariance matrix $\mathbf{C}_p$
(}\DIFaddend Equation~\ref{eq:cov})\DIFaddbegin \DIFadd{,
}\DIFaddend and keep only a given percentage of those with the smallest variance.
...
\DIFaddbegin \subsubsection{\DIFadd{Choice of data weights}}

\DIFadd{The data weight matrix $\mathbf{W}$ (Equation~\ref{eq:constrained}) can be used
...
\DIFadd{In practice, data weights can be calculated based on known data uncertainties
or determined by trial and error.
We have found that the following weights work well in most of the applications
we have undertaken: }\DefaultWeightsF{} \DIFadd{for the total-field anomaly,
}\DefaultWeightsE{} \DIFadd{for the east-derivative, }\DefaultWeightsN{} \DIFadd{for the
north-derivative, and }\DefaultWeightsU{} \DIFadd{for the upward-derivative.
...
\begin{enumerate}
    \item \textbf{Method demonstration:} This test uses a single data window
        ...
        of the Euler inversion method and \DIFdelbegin \DIFdel{it's }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend ability to correctly estimate
    ...
    \item \textbf{Effect of interfering sources:} This test uses \DIFdelbegin \DIFdel{four different
        models
        of a dipolar source and a single data window with an interfering
        dipolar source present in different locations and depths}\DIFdelend \DIFaddbegin \DIFadd{models
        composed of two sources at increasing distances from each other. The
        test is performed for two dipolar sources and also two dykes}\DIFaddend . Its aim
        is to investigate the effect of interfering sources inside the data
        window on the Euler inversion, Euler deconvolution, and
        finite-difference Euler deconvolution results. \DIFaddbegin \DIFadd{We use a single data
        window so that we can understand what happens at each step of the
        moving window procedure.
    }\DIFaddend \item \textbf{Moving window procedure with multiple sources:} This test
...
Also shown are the true source location, the initial source location\DIFdelbegin \DIFdel{(the Euler deconvolution result)}\DIFdelend , and the predicted source location from Euler inversion.
The \DIFaddbegin \DIFadd{initial estimate of the source location was $(x_o=\SynProofEDEast,
y_o=\SynProofEDNorth, z_o=\SynProofEDUp)$ and the base level was $b
= \SynProofEDBase$, which are the Euler deconvolution results.
The }\DIFaddend final Euler inversion prediction of the source location was
...
estimated values by Euler deconvolution (Figure~\ref{fig:proof}\DIFaddbegin \DIFadd{i}\DIFaddend ).

\DIFdelbegin \DIFdel{The convergence of the solution is shown in Figures~\ref{fig:proof}i-j.
The }\DIFdelend \DIFaddbegin \DIFadd{Figure~\ref{fig:proof}i shows the }\DIFaddend error in the estimated source coordinates and base level\DIFdelbegin \DIFdel{are shown in Figure~\ref{fig:proof}i.
The }\DIFdelend \DIFaddbegin \DIFadd{.
We can see from the figure that the }\DIFaddend error in the $x_o$ (easting) and $y_o$
(northing) coordinates, as well as the base level, do not vary greatly from the
initial solution \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{at iteration 0.
}\DIFaddend However, the error in the $z_o$ (upward) coordinate drops from over \qty{1400}{\m} to less than \qty{400}{\m} in two iterations.
...
We ran the Euler inversion method on each data grid \DIFdelbegin \DIFdel{three }\DIFdelend \DIFaddbegin \DIFadd{four }\DIFaddend times, each time
changing the structural index between \DIFaddbegin \DIFadd{zero, }\DIFaddend one, two, and three.
...
(Algorithm~\ref{alg:si} \DIFaddbegin \DIFadd{with $\eta_{min}=\DefaultSIMin$ and
$\eta_{max}=\DefaultSIMax$}\DIFaddend ).
...
\caption{
    ...
    effect of interfering \DIFaddbeginFL \DIFaddFL{dipolar }\DIFaddendFL sources inside the data window on the Euler
    inversion results.
    a-d) \DIFdelbeginFL \DIFdelFL{Noise-corrupted total-field }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Total-field }\DIFaddendFL magnetic anomaly \DIFdelbeginFL \DIFdelFL{for }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{of }\DIFaddendFL four \DIFaddbeginFL \DIFaddFL{out of the }\SynInterfNModels{}
    \DIFaddendFL models, each of which \DIFdelbeginFL \DIFdelFL{include }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{includes }\DIFaddendFL the same central dipole \DIFdelbeginFL \DIFdelFL{but different }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and an }\DIFaddendFL interfering
    \DIFdelbeginFL \DIFdelFL{sources in the form of another }\DIFdelendFL dipolar source \DIFaddbeginFL \DIFaddFL{at different distances from the main source}\DIFaddendFL . Also plotted
    are the estimated positions from Euler deconvolution \DIFaddbeginFL \DIFaddFL{(green square)}\DIFaddendFL ,
    finite-difference Euler deconvolution \DIFaddbeginFL \DIFaddFL{(red diamond)}\DIFaddendFL , and Euler inversion
    \DIFaddbeginFL \DIFaddFL{(orange circle)}\DIFaddendFL .
    \DIFdelbeginFL \DIFdelFL{e}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{e-g}\DIFaddendFL ) The error in the estimated \DIFaddbeginFL \DIFaddFL{eastward, northward, and }\DIFaddendFL upward
    \DIFdelbeginFL \DIFdelFL{coordinate }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{coordinates, respectively, }\DIFaddendFL of the source \DIFdelbeginFL \DIFdelFL{$z_o$ }\DIFdelendFL for each of the Euler methods as
    a function of the \DIFdelbeginFL \DIFdelFL{model number}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{distance between sources}\DIFaddendFL .
    \DIFdelbeginFL \DIFdelFL{f}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{h}\DIFaddendFL ) The estimated structural index $\eta$ for Euler inversion and
    finite-difference Euler deconvolution as a function of the \DIFdelbeginFL \DIFdelFL{model number}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{distance between
    sources}\DIFaddendFL .
\DIFdelbeginFL \DIFdelFL{The true source location is represented by a black triangle, the Euler deconvolution result by a green square, the finite-difference Euler deconvolution result by a red diamond, and the Euler inversion result by an orange circle.
}\DIFdelendFL }
\label{fig:interf}
\end{figure}
...
\caption{
    \DIFaddFL{Data and results from the synthetic data test used to investigate the
    ...
    \DIFaddFL{models, each of which includes the same dyke to the east and an interfering
...
\DIFaddend Another common issue encountered during the application of Euler-based methods
is the presence of interfering sources within the data window.
To test this effect on Euler inversion, we create \DIFdelbegin \DIFdel{four different }\DIFdelend \DIFaddbegin \DIFadd{two different scenarios, one
with two dipoles and another with two dykes.
In both scenarios, we created several }\DIFaddend synthetic total-field anomaly datasets
\DIFdelbegin \DIFdel{.
All contain a main dipole located at $(x_o=\SynInterfTrueEast, y_o=\SynInterfTrueNorth, z_o=\SynInterfTrueUp)$ with a dipole moment amplitude of }%DIFDELCMD < \SynInterfInt{}%%%
\DIFdel{, inclination of }%DIFDELCMD < \SynInterfInc%%%
\DIFdel{, and declination of }%DIFDELCMD < \SynInterfDec%%%
\DIFdel{.
The reference field direction was the same as the dipole moment direction.
Each of
the four models also contain a second dipolar source, simulating an interfering source in the data window, that is located at different places and depths relative to the main dipole.
The total-field anomaly data were generated on regular grids with a spacing of }%DIFDELCMD < \SynInterfSpacing{} %%%
\DIFdel{and at a constant height of }%DIFDELCMD < \SynInterfHeight%%%
\DIFdelend \DIFaddbegin \DIFadd{by varying the distance between the two sources.
No noise was added to these synthetic data in order to isolate the effect of
the interfering source from the effect of random noise}\DIFaddend .
We added to all datasets a base level of \SynInterfTrueBase{}\DIFdelbegin \DIFdel{and pseudo-random Gaussian noise with }%DIFDELCMD < \qty{0}{\nano\tesla} %%%
\DIFdel{mean and }%DIFDELCMD < \SynInterfNoise{} %%%
\DIFdel{standard deviation.
Figures~\ref{fig:interf}a-d show the noise-corrupted total-field anomaly for each of the models.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend On each dataset\DIFdelbegin \DIFdel{we, }\DIFdelend \DIFaddbegin \DIFadd{, we }\DIFaddend ran Euler deconvolution (Equation~\ref{eq:deconv-p}\DIFdelbegin \DIFdel{with structural index }\DIFdelend \DIFaddbegin \DIFadd{) with
the correct structural index for the source (}\DIFaddend $\eta=3$ \DIFaddbegin \DIFadd{for the dipoles and
$\eta=1$ for the dykes}\DIFaddend ), the finite-difference Euler deconvolution method of
\citet{Gerovska2005}, and Euler inversion with the structural index estimation
(Algorithm~\ref{alg:si} \DIFaddbegin \DIFadd{with $\eta_{min}=\DefaultSIMin$ and
$\eta_{max}=\DefaultSIMax$}\DIFaddend ) and data weights of \DefaultWeightsF{} for the
...
upward derivative.
\DIFdelbegin \DIFdel{The estimated easting and northing }\DIFdelend \DIFaddbegin 

\DIFadd{The dipole models contain a main dipole located at $(x_o=\SynInterfTrueEast,
y_o=\SynInterfTrueNorth, z_o=\SynInterfTrueUp)$ with a dipole moment amplitude
of }\SynInterfInt{}\DIFadd{, inclination of }\SynInterfInc\DIFadd{, and declination of
}\SynInterfDec\DIFadd{.
...
}\SynInterfInterfEastMin{} \DIFadd{to }\SynInterfInterfEastMax{}\DIFadd{.
The reference field direction was the same as the dipole moment direction.
The total-field anomaly data were generated on regular grids with a spacing of
}\SynInterfSpacing{} \DIFadd{and at a constant height of }\SynInterfHeight\DIFadd{.
Figures~\ref{fig:interf}a-d show the total-field anomaly of four out of the
}\SynInterfNModels{} \DIFadd{models.
}

\DIFadd{The error in the estimated eastward, northward, and upward }\DIFaddend coordinates are
shown in Figures~\ref{fig:interf}\DIFdelbegin \DIFdel{a-d.
For models 2 and 3, all three methods performed similarly in estimating the horizontal coordinates of the true source .
For models 1 and 4, the Euler deconvolution and }\DIFdelend \DIFaddbegin \DIFadd{e-g.
...
roughly equivalent and both have smaller errors than }\DIFaddend finite-difference Euler
deconvolution \DIFdelbegin \DIFdel{results are comparable,
whilst the
Euler inversion results are closer to the true source location.
Figure~\ref{fig:interf}e shows the }\DIFdelend \DIFaddbegin \DIFadd{for all but the largest distances.
...
\DIFadd{The dyke models contain a main dyke at the east with a center point at
...
of }\SynInterfDykesInt{}\DIFadd{, inclination of }\SynInterfDykesInc\DIFadd{, and declination of
}\SynInterfDykesDec\DIFadd{.
The interfering dyke has a top at $z_o=\SynInterfDykesInterfUp$, with the $x_o$
coordinate varying from
}\SynInterfDykesInterfEastMin{} \DIFadd{to }\SynInterfDykesInterfEastMax{}\DIFadd{.
The reference field direction was the same as the dipole moment direction.
The total-field anomaly data were generated on regular grids with a spacing of
}\SynInterfDykesSpacing{} \DIFadd{and at a constant height of }\SynInterfDykesHeight\DIFadd{.
Figures~\ref{fig:interf-dykes}a-d show the total-field anomaly of four out of
the }\SynInterfDykesNModels{} \DIFadd{models.
}

\DIFadd{The }\DIFaddend error in the estimated \DIFdelbegin \DIFdel{upward coordinate for }\DIFdelend \DIFaddbegin \DIFadd{eastward, northward, and upward coordinates are
...
For the eastward and northward coordinates, the three methods are mostly
compatible.
When the two dykes intersect, }\DIFaddend all three methods \DIFdelbegin \DIFdel{and four models.
The Euler inversion errors are consistently lower than those of
both }\DIFdelend \DIFaddbegin \DIFadd{estimate a horizontal position
at the intersection point.
When they don't intersect, the estimates for all three methods falls outside of
the data window.
This is a well known issue for }\DIFaddend Euler deconvolution methods \DIFdelbegin \DIFdel{.
With the exception of model 2, the Euler inversion error on the upward coordinate are
approximately half those of the Euler deconvolution methods.
Figure~\ref{fig:interf}f shows the estimated structural index }\DIFdelend \DIFaddbegin \DIFadd{because the Hessian
matrix $\mathbf{A}^T\mathbf{A}$ (Equation~\ref{eq:deconv-p}) is ill-conditioned
}\DIFaddend for \DIFaddbegin \DIFadd{2D sources \mbox{%DIFAUXCMD
\citep{Mushayandebvu2004}}\hskip0pt%DIFAUXCMD
.
For the upward coordinate, Euler inversion and Euler deconvolution are
...
}\DIFaddend finite-difference Euler deconvolution \DIFdelbegin \DIFdel{and Euler inversion.
The finite-difference Euler deconvolution method consistently estimated values lower than $\eta=1$.
With the exception of model 2, Euler inversion was able to estimate the correct structural index ($\eta=3$) for all other models}\DIFdelend \DIFaddbegin \DIFadd{estimates incorrect $\eta$ at most
distances, presenting no clear relationship between distance and $\eta$
estimate.
Euler inversion estimates the correct index of $\eta=1$ for all distances}\DIFaddend .
...
(Algorithm~\ref{alg:window} \DIFaddbegin \DIFadd{and Algorithm~\ref{alg:si} with
$\eta_{min}=\DefaultSIMin$ and $\eta_{max}=\DefaultSIMax$}\DIFaddend ), the
...
\DIFaddbegin \DIFadd{It is also notable that there are solutions that outline the simulated dykes
for all three methods.
This seems to be in contradiction of the results presented in
Section~\ref{sec:interf} and the theoretical proof in
\mbox{%DIFAUXCMD
\citet{Mushayandebvu2004}}\hskip0pt%DIFAUXCMD
, which show that the $x_o$ and $y_o$ coordinates
...
\DIFaddend \subsection{Aeromagnetic data from Rio de Janeiro}
\label{sec:rio}
...
\DIFaddbegin \begin{figure}[tb!]
\centering
\includegraphics[width=1\linewidth]{figures/real-data-application-comparison.png}
\caption{
    \DIFaddFL{Results of applying Euler deconvolution and finite-difference Euler
    deconvolution with a window size of }\RioWindowSize{} \DIFaddFL{and a window step of
    }\RioWindowStep{} \DIFaddFL{to the aeromagnetic data from Rio de Janeiro, Brazil.
...
\DIFaddend The geology of Rio de Janeiro state (Southeastern Brazil) consists primarily of
...
We performed the moving-window Euler inversion (Algorithm~\ref{alg:window} \DIFaddbegin \DIFadd{and
Algorithm~\ref{alg:si} with $\eta_{min}=\RioSIMin$ and
$\eta_{max}=\RioSIMax$}\DIFaddend ) on the observed total-field anomaly line data using
...
\DIFaddbegin \DIFadd{We also performed Euler deconvolution with structural indices one, two, and
...
The \DIFaddbegin \DIFadd{Euler inversion }\DIFaddend estimated source positions shown in
...
\DIFaddbegin \DIFadd{When compared to the Euler deconvolution and finite-difference Euler
...
potential-field data\DIFdelbegin \DIFdel{, but under-performs }\DIFdelend \DIFaddbegin \DIFadd{.
It has its limitations }\DIFaddend in real-world scenarios due to its dependence on the
chosen value of the structural index $\eta$ and its sensitivity to
high-frequency noise and signal overlap \DIFaddbegin \DIFadd{from interfering sources}\DIFaddend .
...
\DIFaddbegin \DIFadd{Our method can be applied to gridded and non-gridded data, which can
...
}\DIFaddend The Euler inversion algorithm is computationally efficient because most of the
...
Euler inversion outperforms \DIFdelbegin \DIFdel{other Euler-based methods }\DIFdelend \DIFaddbegin \DIFadd{traditional Euler deconvolution }\DIFaddend in most cases.
...
\DIFaddbegin \DIFadd{As is the case with other Euler deconvolution-based methods, Euler inversion
also suffers from instability when sources are 2D \mbox{%DIFAUXCMD
\citep{Mushayandebvu2004}
}\hskip0pt%DIFAUXCMD
and a lack of support for sources that are defined by multiple points, for
example steps which have a top and bottom \mbox{%DIFAUXCMD
\citep{Gerovska2010}}\hskip0pt%DIFAUXCMD
.
...
\DIFaddend %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{\DIFdelbegin \DIFdel{Open research}\DIFdelend \DIFaddbegin \DIFadd{Data availability statement}\DIFaddend }

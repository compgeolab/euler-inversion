...
The estimation of the depths of the sources of measured anomalies is a common challenge in potential-field geophysics.
One of the most widely used techniques for providing depth estimates is Euler deconvolution \citep{Thompson1982,Reid1990}.
Its widespread adoption is due, in large part, to its low algorithmic complexity and fast computation times, both of which are orders of magnitude smaller than solutions to 3D inverse problems.
...
Euler deconvolution is based on a least-squares solution to Euler's homogeneity equation for idealized sources.
...
Euler deconvolution takes as inputs the potential-field data and its three spatial derivatives as well as the structural index.
...
\item \textbf{Sensitivity to high-frequency noise:} Random noise in the data are usually of high-frequency, which get amplified in the derivative calculations. Since the field derivatives are used in the Jacobian matrix of the least-squares problem, errors in the derivatives will have a large impact on the solution.
\citet{Pasteka2009}, \citet{Saleh2012}, and \citet{Florio2014} recommend using regularized derivatives or other smoothing techniques to reduce the noise amplification and obtain more reliable solutions.
...
The higher the structural index, the larger is the estimated depth.
...
We propose formulating the inverse problem as a non-linear optimization with Euler's equation as a constraint.
...
The dipolar anomalies are associated with post-collisional and alkaline intrusions, many of which are also cut by known outcropping dikes or have known dikes with magnetic signals that significantly overlap with the dipolar anomalies.
...
\begin{itemize}
\item \textbf{Anomaly A:} Has a reversed polarity and linear feature to its north that is not associated with any known dike outcrop.
The linear feature is highlighted by Euler inversion estimates with $\eta=1$ and depth of \qtyrange{300}{400}{\m}, which can be interpreted as a non-outcropping dike.
The dipolar anomaly itself has Euler inversion solutions with $\eta=3$ and depth of \qtyrange{1000}{2000}{\m}.
The solutions in the center of the anomaly present a shallower depth than the solutions to the north and south of the anomaly center.
...
The depth to the center of the anomaly source is likely close to \qty{1000}{\m}.
...
The anomaly is cut by several NE-SW linear features, some of which overlap with known dike outcrops.
The linear feature to the north is associated with Euler inversion results with $\eta=1$ and depths ranging from \qtyrange{300}{600}{\m}, suggesting a non-outcropping dike.
At the center of the anomaly are Euler inversion estimates with $\eta=3$ and depth estimate of approximately \qty{1400}{\m}.
The Euler inversion solutions surrounding these central solutions are likely caused by interferance from other sources.

\item \textbf{Anomaly C:} A dipolar anomaly associated with an outcropping portion of the post-collisional magmatism.
There is a known outcropping dike to the south of the anomaly, which is associated with Euler inversion estimates with $\eta=1$ and depths ranging from \qtyrange{500}{1000}{\m}.
...
There are known outcropping dikes around the main intrusion but they have no discernible magnetic anomalies and no Euler inversion solutions associated with them.
\end{itemize}
...
The main linear features are mostly associated with Euler inversion estimates with $\eta=1$ and shallow depths, particularly where known dike outcrops are located.
...
A test on complex synthetic data from a model of dikes and dipoles with overlapping signals shows that Euler inversion is able to estimate the structural index and position of the sources within expected error bounds when the signal overlap is not larger than the data window.
For deeper dikes in particular, Euler inversion was not able to estimate the correct $\eta=1$, leading to an overestimation of the depths.

We applied Euler inversion to an aeromagnetic dataset from Rio de Janeiro, Brazil, to analyze its performance under real-world scenarios.
Euler inversion was able to locate the NE-SW linear features in the data with an $\eta=1$ which are associated with known dike outcrops.
...
\textbf{Leonardo Uieda:} Conceptualization, Data curation, Formal analysis, Investigation, Methodology, Project administration, Resources, Software, Supervision, Visualization, Writing – original draft.
\textbf{Gelson Ferreira Souza-Junior:} Data curation, Formal analysis, Resources, Software, Visualization, Writing - original draft, Writing - review \& editing.
\textbf{India Uppal:} Data curation, Formal analysis, Investigation, Software, Writing - review \& editing.
\textbf{Vanderlei Coelho Oliveira Jr.:} Conceptualization, Methodology, Writing - review \& editing.

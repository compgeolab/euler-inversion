...
\begin{equation}
  ...
  \begin{bmatrix}
    ...
    - \mathbf{B}_k\mathbf{W}^{-1}\mathbf{B}_k^T \boldsymbol{\lambda}_k
    \\
    \mathbf{A}_k^T\boldsymbol{\lambda}_k
  \end{bmatrix}
  =
...
Notice that Equations~\ref{eq:deltap}~and~\ref{eq:deltad} above does not depend
on $\boldsymbol{\lambda}_k$ or $\boldsymbol{\Delta\lambda}_k$. Thus,
Equation~\ref{eq:deltalambda} does not need to be calculated in practice.
...
An advantage of Euler inversion of Euler deconvolution is its ability to obtain
...
choosing the one that produces the smallest WRMSE. This procedure is summarized
in Algorithm~\ref{alg:si}.
...
Windowing procedure and another algorithm.
